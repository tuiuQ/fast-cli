 ...
 * 创建字体子集
 */
export async function createFontSubset(fontFile: string, options: ISubsetOptions): Promise<void> {
  logger.info(`正在处理字体文件: ${fontFile}`)
  logger.info(`目标语言: ${options.lang}`)

  // 验证语言支持
  if (!(options.lang in charsetMap)) {
    throw new Error(`不支持的语言: ${options.lang}. 支持的语言: ${Object.keys(charsetMap).join(', ')}`)
  }
